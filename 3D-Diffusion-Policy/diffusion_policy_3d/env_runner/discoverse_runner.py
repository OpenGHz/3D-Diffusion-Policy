...
import torch
import tqdm
from diffusion_policy_3d.env import DiscoverseEnv
...
class DiscoverseRunner(BaseRunner):
    def __init__(self,
                 output_dir,
                 eval_episodes=20,
                 max_steps=200,
        ...
        def env_fn():
            return MultiStepWrapper(
                SimpleVideoRecordingWrapper(
                    DiscoverseEnv(task_name=task_name,device=device, 
    ...
    def run(self, policy: BasePolicy):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Discoverse {self.task_name} Pointcloud Env",
